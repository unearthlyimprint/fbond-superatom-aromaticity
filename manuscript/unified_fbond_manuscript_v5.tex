...
density $f_e = N_D/N_e$ as a complementary intensive measure. This
intensive measure reveals a clear separation: small clusters
(\ce{Al4}, \ce{B12}, \ce{B6N6}) exhibit $f_e \approx 0.07$, while
...
$f_e = N_D/N_e$ as an intensive measure that reveals distinct correlation
...
$N_D$.\cite{arda2025fbond} Recognizing its mathematical
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrrrcc}
    ...
    \ce{Al4^{4-}} (triplet)        &  40 &  32 &   6 &   6 (100\%)& 0.364 & 6.00 & 0.188 \\
...
The per-electron correlation density $f_e = N_D/N_e$
separates the closed-shell systems into two distinct regimes:

\textbf{Small cluster regime} ($f_e \approx 0.07$):
\ce{Al4^{2-}}, \ce{Al4^{4-}}, \ce{B12} (both isomers), and \ce{B6N6}
all exhibit per-electron correlation densities clustered tightly around
$f_e = 0.071$--$0.083$. Despite spanning different elements (Al, B, N),
different topologies (planar, icosahedral), and different bonding
characters (aromatic, antiaromatic, electron-deficient),
these systems share a common intrinsic correlation density.
The slight outlier is icosahedral \ce{B12} ($f_e = 0.083$),
The per-electron correlation density $f_e = N_D/N_{\text{corr}}$
resolves this ambiguity. Small clusters (\ce{Al4}, \ce{B12}, \ce{B6N6})
exhibit $f_e \approx 0.08$--$0.14$, indicating that roughly 8--14\% of each
correlated electron's occupation deviates from idempotency. Metallic superatoms,
by contrast, show $f_e \approx 0.04$--$0.05$, indicating diluted, extensive
correlation spread over the entire cluster rather than concentrated in a
few strongly correlated orbitals. The icosahedral \ce{B12} cage
($f_e = 0.139$) stands out as having the highest $f_e$ among closed-shell
systems, reflecting the orbital degeneracies imposed by $I_h$ symmetry.
The nearly identical $f_e$ values for
the two superatoms ($0.048$ vs.\ $0.044$, a 8\% difference) suggest that
$f_e$ converges toward a characteristic value for the \ce{Cs3Al_n^-}
superatom family.

The triplet \ce{Al4^{4-}} ($f_e = 0.188$) stands apart as an open-shell
system where all six active orbitals are near half-occupation, yielding
the maximum possible correlation density within its small active space.
...
with all six active natural orbitals near half-occupation ($n_i \approx 1.0$),
it yields $N_D = 6.0$---the theoretical maximum for six
orbitals. This triplet lies \SI{7.5}{\kcal\per\mol} below the singlet,
...
\begin{itemize}
  ...
  \item $f_e \gg 0.1$:
    Open-shell systems with strongly occupied singly-occupied orbitals.
    The triplet \ce{Al4^{4-}} ($f_e = 0.15$) exemplifies this regime.
\end{itemize}
...
on a neutral-atom quantum processor. We performed analog quantum
...
\begin{table}[H]
  ...
  \caption{%
    ...
    $S_E^C$ = classical CCSD $S_{E,\text{max}}$ for comparison.
...
\begin{figure}[H]
  ...
  \caption{%
    ...
    the entanglement \emph{trends} are remarkably consistent between
...
\begin{itemize}
    ...
    having higher classical correlation ($f_e = 0.083$ vs.\ $0.074$). This reversal
...
$f_e = N_D/N_e$ as a complementary intensive descriptor.
...
\begin{enumerate}
  \item All systems studied exhibit substantial aggregate correlation
    ...
    $N_D$ ranging from 3.84 to 7.10 for closed-shell
  ...
  \item The per-electron correlation density $f_e$ cleanly separates two
    regimes: small clusters ($f_e \approx 0.08$--$0.14$) and metallic superatoms
    ($f_e \approx 0.04$--$0.05$). The superatoms exhibit ``wide but shallow''
    correlation---96\% of orbitals are fractionally occupied, but the
    per-electron correlation density is roughly half that of small clusters.
  ...
  \item \textbf{Molecular Topology on Quantum Hardware:} Analog simulation
    ...
    wavefunction. The observation that entanglement scales with graph
    connectivity rather than register size suggests that molecular
    bonding networks encode topological information accessible to
