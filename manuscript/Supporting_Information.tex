...
(1~\AA{} $\to$ 3~$\mu$m) to satisfy the hardware's minimum
atom separation constraint ($R > \SI{5}{\micro\meter}$).
...
\begin{table}[H]
  \centering
  \caption{%
    Classical $S_{E,\text{max}}$ and Rydberg entanglement
...
\ce{Al4^{4-}}, confirming that molecular graph topology is
faithfully encoded in the Rydberg interaction graph.
...
``active spaces'' artificially suppresses $N_D$ by up
