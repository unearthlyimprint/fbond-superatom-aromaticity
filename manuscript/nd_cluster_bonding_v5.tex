% ============================================================================
% N_D Cluster Bonding Manuscript v5 — Extended Quantum Validation
% ============================================================================
...
PySCF's default Gaussian basis treatment was used; for systems
containing $d$-functions, this employs Cartesian Gaussians ($6d$),
yielding slightly more basis functions than the spherical-harmonic
convention ($5d$). The resulting correlated orbital counts $M$ are
listed in Table~\ref{tab:main}.
...
\begin{table}[H]
  ...
  \caption{%
    ...
    $M_{\text{frac}}$ = orbitals deviating from idempotency
    ($|n_i - 2| > 10^{-6}$ for occupied, $n_i > 10^{-6}$ for virtual);
  ...
  \begin{tabular}{lrrrrrcc}
    ...
    System & $N_e$ & $N_{\text{corr}}$ & $M$ & $M_{\text{frac}}$ & $|E_{\text{corr}}|$ (Ha)
      & $N_D$ & $f_e$ \\
    \midrule
    \ce{C6H6} (benzene)            &  42 &  30 & 114 &  75 (66\%) & 0.821 & 2.49 & 0.083 \\
    ...
    \ce{B12N12} (cage)             & 144 &  96 & 360 & 104 (29\%) & 2.888 & 7.18 & 0.075 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/fig1_nd_bar.pdf}
  \caption{%
...
systems in this dataset when total correlation is measured;
earlier $N_D$ values of $< 0.001$ for aluminum clusters reflect
the use of small active orbital windows that isolate static correlation
but exclude the extensive dynamic correlation tail
...
\ce{C6H6}, \ce{Al4^{2-}}, \ce{Al4^{4-}}, \ce{B12} (both isomers), \ce{B6N6},
and \ce{B12N12}
...
\textbf{Metallic superatom regime} ($f_e \approx 0.03$--$0.05$):
Metallic superatoms show
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/fig3_nd_vs_ne.pdf}
  \caption{%
...
Because $N_D$ includes dynamic correlation contributions from
the virtual orbital space, the ratio of virtual orbitals to correlated
electrons ($M_{\text{virt}}/N_{\text{corr}}$) can influence both $N_D$
and $f_e$. In our dataset, light-element systems (\ce{B12}, \ce{B6N6})
have $M_{\text{virt}}/N_{\text{corr}} \approx 2.0$--$2.5$ with def2-SVP,
while the ECP-treated superatoms have
$M_{\text{virt}}/N_{\text{corr}} \approx 1.2$--$1.3$, and
\ce{Au13^-} has $M_{\text{virt}}/N_{\text{corr}} \approx 0.5$.
This basis-set imbalance means that a portion of the
$f_e$ difference between the small-cluster and superatom regimes may
reflect unequal virtual-space completeness rather than purely physical
differences in correlation character.
However, within each bonding family (e.g., \ce{Cs3Al8^-} vs.\
\ce{Cs3Al_{12}^-}, or planar vs.\ icosahedral \ce{B12}), the
$M_{\text{virt}}/N_{\text{corr}}$ ratios are comparable,
so the \emph{within-family} trends in $f_e$ are robust.
A systematic basis-set convergence study (e.g., def2-TZVP)
would be needed to establish whether the absolute $f_e$ separation
between regimes narrows at the complete basis-set limit.

The \ce{B12N12} cage ($N_D = 7.18$, $f_e = 0.075$, 360 correlated NOs)
...
it has the highest $N_D$ overall. Despite its massive size (144 electrons),
the rigid covalent cage yields $f_e = 0.075$, placing it squarely in the
small-cluster correlation regime ($f_e \approx 0.08$) rather than the
metallic regime ($f_e \approx 0.04$). This implies that a relatively small
...
This finding has important implications for NOON-based correlation diagnostics.
When the goal is to measure \emph{total} electron correlation ($N_D$, encompassing
both static and dynamic contributions), restricting analysis to a small
active space of frontier orbitals can dramatically underestimate the result.
We emphasize that this does \emph{not} imply that active-space methods are
flawed for their intended purpose: CASSCF and related multireference methods
deliberately truncate to a small orbital window precisely to isolate
\emph{static} (nondynamic) correlation, and a small $N_D^{\text{trunc}}$ from
an active-space analysis correctly indicates weak \emph{static} correlation
even when the full-space $N_D^{\text{full}}$ is large.
The distinction is that $N_D^{\text{full}}$ measures aggregate correlation
(predominantly dynamic), while active-space $N_D^{\text{trunc}}$ measures
static correlation---these are complementary diagnostics probing different
physical quantities.
...
using a system-dependent scale factor chosen to satisfy the hardware's minimum atom
separation constraint ($R_{\text{min}} > \SI{5}{\micro\meter}$).
For systems with short bonds (e.g., \ce{C6H6} with $R_{\text{CC}} = 1.39$~\AA,
\ce{B12} with $R_{\text{BB}} \approx 1.6$~\AA), a scale factor of 4~$\mu$m/\AA{}
was used; for systems with larger interatomic distances (\ce{Al4}, \ce{Cs3Al8^-}),
3~$\mu$m/\AA{} was sufficient (see Supporting Information for per-system
scaling factors and minimum separations).
...
\begin{table}[H]
  ...
  \caption{%
    ...
    $N_q$ = number of qubits (atoms in the register);
    $S_E^Q$ = maximum single-site entropy from the Rydberg state;
    $S_E^C$ = classical single-orbital entanglement entropy evaluated
    from the HOMO natural orbital occupation,
    $S_E^C = -p\ln p - (1-p)\ln(1-p)$ with $p = n_{\text{HOMO}}/2$,
    representing the \emph{frontier-orbital} entanglement for comparison.
    Note: this frontier-only $S_E^C$ is distinct from the full-space
    $S_{E,\text{max}}$ reported in SI Table~4, which evaluates the entropy
    over \emph{all} natural orbitals and is much larger
    (e.g., 0.285~nats for \ce{Cs3Al8^-}).
    Systems ordered by bonding character. Note that $S_E^Q$ and $S_E^C$
    arise from fundamentally different Hamiltonians.
    \textsuperscript{\emph{a}}For \ce{B12N12}, the 24-atom cage was
    represented by a 16-atom register using only symmetry-unique
    sites from the $T_d$ structure, preserving the connectivity
    topology while remaining within emulator constraints.
  }
  \label{tab:quantum}
  \begin{tabular}{llccc}
    \toprule
    ...
    \ce{B12N12}     & Heteroatomic cage   & 16\textsuperscript{\emph{a}} & 0.581 & 0.035 \\
    \ce{Au13^-}     & Noble-metal core    & 13 & 0.630 & 0.040 \\
    \ce{Cs3Al8^-}   & Metallic superatom  & 11 & 0.674 & 0.013\textsuperscript{\emph{b}} \\
    \bottomrule
  \end{tabular}

  \smallskip\noindent
  \textsuperscript{\emph{b}}The numerical coincidence between
  $S_E^C = 0.013$ and the truncated-space $N_D^{\text{trunc}} = 0.013$
  (Table~\ref{tab:truncation}) is accidental: $S_E^C$ is a single-orbital
  entropy evaluated from an orbital with $n_i \approx 1.997$
  near the Fermi level, while $N_D^{\text{trunc}}$ is a summation over
  20 frontier orbitals. The full-space $S_{E,\text{max}} = 0.285$~nats
  (SI Table~4) arises from deep-valence orbitals with $n_i \approx 1.83$.
\end{table}
...
\begin{itemize}
    ...
    geometry loses its three-dimensional connectivity upon projection.
    We emphasize that 2D projection of 3D structures necessarily alters
    the $1/R^6$ interaction graph by collapsing the third spatial dimension,
    bringing some atom pairs artificially closer together while leaving
    others relatively unchanged. For icosahedral \ce{B12}, the mean
    pairwise distance decreases by $\sim$15\% upon projection,
    distorting the interaction heterogeneity.
    The $S_E^Q$ values for projected 3D structures should therefore be interpreted
    as probing the \emph{projected} topology rather than the true 3D molecular graph.
    This limitation is intrinsic to current 2D neutral-atom register architectures and
    does not affect the planar systems (\ce{C6H6}, \ce{Al4}, planar \ce{B12}, \ce{B6N6}),
    which map without distortion.
    The \ce{B12N12} cage ($S_E^Q = 0.581$, 16 qubits) falls between the two
...
the specific \emph{geometric encoding of different bonding topologies}---the
particular spatial arrangement of atoms characteristic of aromatic, metallic,
and cage-like structures---embedded in the molecular coordinate graph.
The topology is necessary (as the
framework to define the interaction Hamiltonian) but not sufficient;
the precise coordinate geometry of each system modulates how efficiently the
adiabatic pulse generates entanglement across the graph.
We note that the Rydberg Hamiltonian has no explicit knowledge of
element types, orbital hybridization, or electron count; any
topology-dependent signatures arise solely from the distance matrix
encoded in the $1/R^6$ interactions.
...
\begin{figure}[H]
  ...
  \caption{%
    ...
    the specific coordinate geometry of different bonding
    topologies---not mere graph connectivity---determines
...
\begin{enumerate}
  \item All systems studied exhibit substantial aggregate correlation
    ...
    ($< 0.001$) in aluminum clusters reflect the use of small active
    orbital windows that capture static correlation but exclude the
    dynamic correlation tail; the full-space $N_D$ is orders of
    magnitude larger.

  \item The per-electron correlation density $f_e$ cleanly separates two
    regimes: small covalent clusters and cages ($f_e \approx 0.07$--$0.14$) and
    metallic superatoms ($f_e \approx 0.03$--$0.05$).
    The superatoms exhibit ``wide but shallow''
  ...
  \item \textbf{Molecular Topology on Quantum Hardware:} Analog simulation
    ...
    reflects the specific coordinate geometry characteristic of different
    bonding topologies, suggests that molecular distance matrices encode
    topological information accessible to analog quantum hardware beyond what
    simple structural descriptors capture. We note that 2D projection of 3D
    structures introduces systematic distortions, and that the Rydberg
    Hamiltonian has no direct knowledge of electronic structure; the observed
    topology dependence arises solely from the atomic distance matrix.
\end{enumerate}
